...
import { staffComplaintsApi } from '@/lib/api';
import { StaffComplaint } from '@/types';
...
export default function StaffComplaintsPage() {
  ...
  const { data: complaints = [], isLoading } = useQuery({
    ...
    queryFn: () => staffComplaintsApi.list().then((r) => r.data),
  });

  const createMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => staffComplaintsApi.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Record<string, unknown> }) => staffComplaintsApi.update(id, data),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['staff-complaints'] }),
  });
    ...
    <div>
      ...
      <div className="p-6 space-y-4">
        <div className="flex justify-end">
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader><DialogTitle>Staff Complaint</DialogTitle></DialogHeader>
        ...
        {isLoading ? (
          ...
          <p className="text-center py-16 text-muted-foreground text-sm">No complaints filed</p>
        ) : (
          <div className="space-y-2">
            {complaints.map((c: StaffComplaint) => (
              <div key={c.id} className="p-4 border rounded-lg">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium">{c.name} {c.department && <span className="text-muted-foreground">— {c.department}</span>}</p>
                  ...
                  {canManage ? (
                    ...
                    <Badge className={`text-xs ${statusColor(c.status)} shrink-0`} variant="outline">{c.status.replace(/_/g, ' ')}</Badge>
                  )}
                </div>
                {c.review_comments && (
                  <div className="mt-2 p-2 bg-muted/30 rounded text-xs">
                    <span className="font-medium">Review: </span>{c.review_comments}
                  </div>
                )}
              </div>
            ))}
